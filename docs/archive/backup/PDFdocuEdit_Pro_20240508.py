...
class MergePDFsDialog(QDialog):
    ...
    def add_list_item(self, path):
        ...
        item_text = f"{file_name} (頁數: {page_count}, 大小: {file_size_mb:.2f} MB)"
        self.list_widget.addItem(item_text)

    def remove_pdf(self):
        for item in self.list_widget.selectedItems():
            item_text = item.text()
            file_name = item_text.split(" (")[0]  # 提取文件名
            file_path = next(path for path in self.file_paths if os.path.basename(path) == file_name)
    ...
    def print_direct(self, printer):
        ...
        try:
            ...
            for page_num in range(self.doc.page_count):
                ...
                # 计算图像居中位置，考虑打印机边距
                printable_area = printer.pageRect()  # 获取打印机可打印区域
                image_rect = QRect(0, 0, scaled_image.width(), scaled_image.height())
                image_rect.moveLeft(printable_area.left())
