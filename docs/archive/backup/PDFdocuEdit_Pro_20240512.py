...
class READMEDialog(QDialog):
    def __init__(self, parent=None):
        ...
        readme_content = """
PDFDocuEdit Pro - README

PDFDocuEdit Pro is a comprehensive PDF editing and management tool designed to streamline your workflow and enhance your productivity. With a user-friendly interface and a wide array of features, it empowers you to effortlessly view, edit, and manipulate PDF documents.

Key Features:

View and Navigate: Seamlessly navigate through PDF documents with ease. Zoom in and out, jump to specific pages, and view document properties.
Edit and Annotate:  Rotate pages, insert new pages from other PDFs, extract specific pages, and delete unwanted pages. 
Merge and Split: Combine multiple PDF files into a single document or split a large PDF into smaller, more manageable files.
Text Extraction: Extract text from selected areas within a PDF and export it to Excel for further analysis or editing.
Search and Highlight: Quickly locate specific text within your document and highlight important information.
Page Sorting: Reorder pages within your PDF document to create a customized structure.
Convert to Word: Convert PDF documents to editable Word files for easy editing and collaboration.
Security: Encrypt and decrypt PDF files to protect sensitive information.
Statistical Page Count: Generate a comprehensive report with page counts and file details for all PDFs within a folder.

Benefits:

Enhanced Efficiency: Streamline your PDF editing tasks and save valuable time.
Improved Accuracy: Ensure precision and accuracy in your document edits.
Increased Productivity: Accomplish more with your PDF documents in less time.
Greater Flexibility: Customize your PDFs to meet your specific needs.
Enhanced Security: Protect your confidential information with password encryption.
...
1. Open a PDF document using the "Open" button or by dragging and dropping the file onto the application window.
2. Explore the various features and tools available in the menu and toolbar.
3. Utilize the intuitive interface to edit, manipulate, and manage your PDF documents.

Support:

For any questions or technical support, please contact Andy Leung at andy846@gmail.com.

We hope PDFDocuEdit Pro empowers you to work with PDFs more effectively and efficiently. Enjoy the experience!
...
class SplitOptionsWidget(QWidget):
    ...
    def initUI(self):
        ...
        # 第一行：保存文件位置
        save_location_layout = QHBoxLayout()  # 创建水平布局
...
class PrintOptionsWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.setupOffsetDisplay(layout)
        layout.addStretch(1)
    ...
    def setupOffsetDisplay(self, layout):
        group_box = QGroupBox("Offset Info")
        group_layout = QVBoxLayout()
        self.offset_info_display = QTextEdit()
        self.offset_info_display.setReadOnly(True)
        self.offset_info_display.setFixedHeight(100)  # 设置固定高度
        group_layout.addWidget(self.offset_info_display)
        group_box.setLayout(group_layout)
        layout.addWidget(group_box)

    def updateOffsetInfo(self, offset_left, offset_top, offset_right, offset_bottom):
        offset_info_text = f"Offsets - Left: {offset_left} mm, Top: {offset_top} mm, Right: {offset_right} mm, Bottom: {offset_bottom} mm"
        self.offset_info_display.setText(offset_info_text)
...
class PDFViewer(QMainWindow):
    ...
    def initUI(self):
        ...
        self.btn_open.setIcon(QIcon(resource_path('App_icon/file.png')))
        ...
        self.btn_save.setIcon(QIcon(resource_path('App_icon/Save.png')))
        ...
        self.btn_pdf_info = QPushButton('PDF Info', self)
        self.btn_pdf_info.setIcon(QIcon(resource_path('App_icon/info.png')))
        ...
        self.btn_print.setIcon(QIcon(resource_path('App_icon/print.png')))
        ...
        self.btn_show_readme = self.setup_button('Readme', self.show_readme, resource_path('App_icon/readme.png'))
        self.left_button_layout.addWidget(self.btn_show_readme)
        

        # 创建并设置 Dock Widget
        self.menu_dock = QDockWidget("Menu", self)
        self.menu_dock.setWidget(left_button_widget)
        ...
        pdf_management_menu.addAction(Batch_convert_MSOFFICE_to_PDF_action)
...
""")
        ...
        self.page_num_edit.setMinimumSize(50, 28)
        self.page_num_edit.setMaximumSize(50, 28)
    ...
    def button_style(self, normal_color, hover_color):
        style = f"background-color: {normal_color};"
        style += f"QPushButton:hover {{ background-color: {hover_color}; }}"
        return style
    ...
    def parse_page_range(self, page_range):
        ...
        for part in page_range.split(','):
            ...
            if '-' in part:
                ...
                if start > end:
                    QMessageBox.warning(self, "錯誤", "起始頁碼不能大于结束頁碼！")
                ...
                try:
                    page_num = int(part)
                    if page_num < 1:
                        QMessageBox.warning(self, "錯誤", "頁碼不能小于1！")
                        return None  # 返回 None 表示解析失敗
    ...
    def start_printing(self):
        try:
            ...
            right_offset = self.print_options_widget.right_offset_spin.value()
            bottom_offset = self.print_options_widget.bottom_offset_spin.value()
            ...
            self.print_direct(printer, left_offset, top_offset, right_offset, bottom_offset)
    ...
    def print_direct(self, printer, left_offset, top_offset, right_offset, bottom_offset):
        ...
        try:
            ...
            for page_num in range(self.doc.page_count):
                ...
                qimage = QImage(pixmap.samples, pixmap.width, pixmap.height, pixmap.stride, QImage.Format_RGB888)
                scale_x = paper_rect.width() / qimage.width()
                scale_y = paper_rect.height() / qimage.height()
                ...
                image_rect.moveCenter(paper_rect.center())
