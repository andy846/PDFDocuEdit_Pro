...
class ContextPanel(QFrame):
    ...
    annotationScanRequested = pyqtSignal()
    ...
    def _build_annotate(self) -> None:
        ...
        self._annotation_scan.setToolTip("Scan all pages on demand; opening a document does not scan its annotations.")
        self._annotation_scan.clicked.connect(self.annotationScanRequested.emit)
        layout.addWidget(self._annotation_scan)
        self._annot_tabs.currentChanged.connect(lambda index: self.annotationScanRequested.emit() if index == 1 else None)
