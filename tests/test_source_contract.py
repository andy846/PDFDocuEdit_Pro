...
def test_release_metadata_is_v2_5_5() -> None:
    assert APP_VERSION == "2.5.11"
    assert "Copyright © 2026 Andy Leung" in COPYRIGHT_NOTICE
    assert 'version = "2.5.11"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert '#define MyAppVersion "2.5.11"' in (
        ROOT / "installer/PDFDocuEditPro.iss"
    ).read_text(encoding="utf-8")
    assert 'VERSION = "2.5.11"' in (ROOT / "scripts/build.py").read_text(encoding="utf-8")
    assert "filevers=(2, 5, 11, 0)" in (
