...
class PdfEngine:
    ...
    def save(self, path: str | os.PathLike[str] | None = None) -> Path:
        ...
        handle, temp_name = tempfile.mkstemp(prefix=f".{target.stem}-", suffix=".pdf", dir=target.parent)
        os.close(handle)
        try:
    ...
    def get_document(self) -> fitz.Document | None: return self._doc
    def get_original_path(self) -> str | None: return str(self._original_path) if self._original_path else None
    def get_temp_path(self) -> str | None: return str(self._temp_path) if self._temp_path else None
    def get_page_count(self) -> int: return self.page_count
    ...
    def insert_pages(self, source_path: str, pages: list[int], position: int) -> None:
        ...
        with fitz.open(source_path) as source:
            ...
            invalid = sorted({page + 1 for page in pages if not 0 <= page < source.page_count})
            if invalid:
    ...
    @_locked
    def insert_blank_pages(self, count: int, position: int, width: float, height: float) -> None:
        doc = self._require_document()
        insert_at = min(max(0, position), doc.page_count)
    ...
    @_locked
    def repeat_insert_pages(self, source_path: str, pages: list[int], interval: int) -> None:
        """Insert source pages after every N pages of the original document."""
        doc = self._require_document()
        if interval < 1 or not pages:
            raise PdfEngineError("A repeat interval and at least one source page are required.")
        ...
        with fitz.open(source_path) as source:
            ...
            for boundary in range(interval, original_count + 1, interval):
                ...
                inserted += len(valid)
        if not inserted:
            raise PdfEngineError("The repeat interval does not create any insertion points.")
        self._is_modified = True
    ...
    def delete_pages(self, pages: Iterable[int]) -> None:
        doc = self._require_document()
        valid = sorted({page for page in pages if 0 <= page < doc.page_count}, reverse=True)
        if len(valid) >= doc.page_count:
            raise PdfEngineError("A PDF must contain at least one page.")
        for page_num in valid:
            doc.delete_page(page_num)
        self._is_modified = bool(valid) or self._is_modified

    @_locked
    def extract_pages(self, pages: Iterable[int], output_path: str | os.PathLike[str]) -> Path:
        doc = self._require_document()
        valid = list(dict.fromkeys(page for page in pages if 0 <= page < doc.page_count))
        if not valid:
    ...
    def rotate_pages(self, pages: Iterable[int], angle: int) -> None:
        ...
        for page_num in pages:
            if 0 <= page_num < doc.page_count:
                ...
                changed = True
        self._is_modified = changed or self._is_modified
    ...
    def organize_pages(self, new_order: list[int], rotations: dict[int, int]) -> None:
        ...
        if len(set(new_order)) != len(new_order) or any(
            ...
            raise PdfEngineError("The page arrangement contains invalid or duplicate pages.")
        doc.select(new_order)
    ...
    def split_pdf(
        ...
        stem = prefix or (self._original_path.stem if self._original_path else "document")
        ...
        for start, end in ranges:
            ...
            if first > last:
                continue
            page_label = f"page_{first + 1}" if first == last else f"pages_{first + 1}-{last + 1}"
        ...
        if len(set(targets)) != len(targets):
            raise PdfEngineError("The selected ranges create duplicate output filenames.")
    ...
    def search_text(self, text: str, page_num: int | None = None) -> list[tuple[int, list[fitz.Rect]]]:
        doc = self._require_document()
    ...
    def extract_text(self, page_num: int | None = None) -> str:
        ...
        if page_num is not None:
            return doc.load_page(page_num).get_text() if 0 <= page_num < doc.page_count else ""
        return "\n\n".join(doc.load_page(index).get_text() for index in range(doc.page_count))
...
def search_pdf_file(
    ...
    *,
    ...
    with fitz.open(path) as doc:
        ...
        return _search_in_document(
            doc, text, case_sensitive=case_sensitive, whole_word=whole_word
        )
...
def _search_words(page: fitz.Page, query: str, case_sensitive: bool) -> list[fitz.Rect]:
    ...
    targets = [
        token.strip(strip_chars) if case_sensitive else token.strip(strip_chars).casefold()
        for token in tokens
    ]
...
def parse_page_range(value: str, page_count: int | None = None) -> list[int]:
    ...
    pages = {page for page in pages if page >= 0 and (page_count is None or page < page_count)}
    return sorted(pages)
